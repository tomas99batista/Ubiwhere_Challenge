...
class OccurrenceAPITest(APITestCase):
    def setUp(self):
        self.normal_user = User.objects.create_user(
            username='normal_user_tests', email='normal_user_tests@example.com', password='normal_user_tests')
        self.super_user = User.objects.create_user(
            username='super_user_tests', email='super_user_tests@example.com', password='super_user_tests')
    ...
    def login_normal_user(self):
        ...
        resp = self.client.post(
            url, {'username': 'normal_user_tests', 'password': 'normal_user_tests'}, format='json')
    ...
    def login_super_user(self):
        url = reverse('login')
        resp = self.client.post(
            url, {'username': 'super_user_tests', 'password': 'super_user_tests'}, format='json')
    ...
    def test_registration(self):
        ...
        resp = self.client.post(
            url, {'username': 'user_register', 'password': 'user_register'}, format='json')
...
#                                format='json')
#        
#        resp = self.client.get(reverse('get_all_occurrences'),
#                                format='json')
#        print(resp.data)
#
#        resp = self.client.patch("api/occurrence/5/",
